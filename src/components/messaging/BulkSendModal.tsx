...
export default function BulkSendModal({ contacts, onClose, onSendBulk }: BulkSendModalProps) {
  ...
  return (
    <div id="bulk-send-modal" className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
      <div className="bg-cream-100 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 flex flex-col max-h-[85vh]">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-cream-200">
          <div>
        ...
        <div className="p-4 border-b border-gray-100 bg-cream-100 flex flex-col sm:flex-row gap-3 items-center justify-between">
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-cream-100 divide-y divide-gray-50 max-h-[30vh]">
          {filtered.length === 0 ? (
            ...
            filtered.map((c) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2.5 rounded-xl transition-all text-left mt-1 ${isSelected ? 'bg-indigo-50/40 border border-indigo-100' : 'hover:bg-cream-200 border border-transparent'}`}
                >
                  <div className="flex items-center gap-3">
                    {c.foto_perfil ? (
                      ...
                      <div className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-neutral-400 border border-gray-200 font-bold text-sm">
                  ...
                  <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${isSelected ? 'bg-indigo-600 border-indigo-600 text-cream-100' : 'border-gray-300'}`}>
        ...
        <form onSubmit={handleSend} className="p-4 border-t border-gray-100 bg-cream-200">
          ...
          <div className="flex gap-2">
            <textarea
              ...
              className="flex-1 p-3 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-cream-100 resize-none"
            />
            <button
              type="submit"
              disabled={selectedIds.length === 0 || !message.trim() || loading}
              className="self-end p-3 rounded-2xl bg-indigo-600 text-cream-100 hover:bg-indigo-700 disabled:opacity-50 disabled:hover:bg-indigo-600 transition-all shadow-md active:scale-95"
            >
