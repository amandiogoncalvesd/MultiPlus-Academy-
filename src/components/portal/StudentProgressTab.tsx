...
export default function StudentProgressTab({ currentUser }: StudentProgressTabProps) {
  ...
  if (loading) {
    return (
      <div className="bg-white p-12 rounded-3xl border border-gray-150 flex flex-col items-center justify-center space-y-3">
        <Loader2 className="w-8 h-8 animate-spin text-[#0A2E5D]" />
        <p className="text-xs text-gray-400 font-mono">A compilar estatísticas de progresso no Supabase...</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-150 flex flex-col justify-between hover:border-[#C89B3C]/20 transition-all">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[8px] font-mono text-gray-400 uppercase tracking-widest block mb-1">Aulas Concluídas</span>
              <span className="text-2xl font-serif font-black text-[#0A2E5D]">{completedCount} de {totalLessons}</span>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center text-[10px] font-mono">
            <span className="text-gray-400">DESEMPENHO CURRICULAR:</span>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-150 flex flex-col justify-between hover:border-[#C89B3C]/20 transition-all">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[8px] font-mono text-gray-400 uppercase tracking-widest block mb-1">Média de Quizzes</span>
              <span className="text-2xl font-serif font-black text-[#0A2E5D]">{avgScore > 0 ? `${avgScore}%` : 'Sem Notas'}</span>
            </div>
            <div className="p-2 bg-[#0A2E5D]/5 text-[#C89B3C] rounded-xl border border-[#0A2E5D]/10">
              <Award size={18} />
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center text-[10px] font-mono">
            <span className="text-gray-400">STATUS DA AVALIAÇÃO:</span>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-150 flex flex-col justify-between hover:border-[#C89B3C]/20 transition-all">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[8px] font-mono text-gray-400 uppercase tracking-widest block mb-1">Rendimento de Ofensiva</span>
              <span className="text-2xl font-serif font-black text-[#0A2E5D]">{streak} dias ativos</span>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center text-[10px] font-mono">
            <span className="text-gray-400">RECORDE DE OFENSIVA:</span>
            <span className="text-[#C89B3C] font-black">{currentUser?.longestStreak || streak} DIAS</span>
          </div>
        </div>
                ...
                <div key={idx} className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    {bench.status ? (
                      ...
                      <span className="text-[9px] font-mono text-gray-400">Pendente de conclusão</span>
                    )}
                  </div>
                </div>
              ))}
