...
export default function InstructorDashboardTab({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Greetings Block */}
      <div className="bg-[#0A2E5D] text-white p-6 sm:p-8 rounded-3xl relative overflow-hidden border border-[#C89B3C]/25 shadow-sm text-left">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-black m-0 text-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          <button
            ...
            className="p-4 rounded-2xl bg-white border border-gray-150 text-left hover:border-[#C89B3C]/40 transition-all cursor-pointer flex flex-col justify-between h-32 hover:shadow-md border-b-2 hover:border-b-[#C89B3C] select-none"
          >
            ...
            <div className="mt-2 text-left">
              <span className="text-xl sm:text-2xl font-serif font-black text-[#0A2E5D] dark:text-neutral-900 block leading-none">
                {kpi.value}
              </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-8 bg-white p-6 rounded-3xl border border-gray-150 text-left flex flex-col justify-between min-h-[350px]">
          <div className="flex justify-between items-center border-b border-gray-100 pb-3">
            <div>
              <span className="text-[9px] font-mono text-gray-400 uppercase font-black tracking-widest block">Relações de Frequência LMS</span>
              <h3 className="text-base font-serif font-bold text-[#0A2E5D] m-0">Análise Temporal de Presença & Aprendizado</h3>
            </div>
              ...
              <svg viewBox="0 0 500 120" className="w-full h-full overflow-visible" preserveAspectRatio="none">
                <defs>
                  <linearGradient id="chartGradient" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <stop offset="100%" stopColor="#0A2E5D" stopOpacity="0.0" />
                  </linearGradient>
                </defs>
                ...
                {activeMetricChart === 'completion' ? (
                  <>
                    ...
                    <circle cx="150" cy="70" r="5" fill="#0A2E5D" stroke="#C89B3C" strokeWidth="2" />
                    <circle cx="300" cy="45" r="5" fill="#0A2E5D" stroke="#C89B3C" strokeWidth="2" />
                    <circle cx="450" cy="10" r="5" fill="#0A2E5D" stroke="#C89B3C" strokeWidth="2" />
                  </>
                ) : (
                  <>
                    <path
                    ...
                    />
                    {/* Points */}
                    <circle cx="150" cy="60" r="5" fill="#C89B3C" stroke="#0A2E5D" strokeWidth="2" />
                    <circle cx="300" cy="55" r="5" fill="#C89B3C" stroke="#0A2E5D" strokeWidth="2" />
                    <circle cx="450" cy="32" r="5" fill="#C89B3C" stroke="#0A2E5D" strokeWidth="2" />
                  </>
                )}
              </svg>
              
              {/* Graph axis coordinates labels */}
              <div className="absolute inset-x-0 bottom-0 flex justify-between text-[9px] font-mono text-gray-400 pt-2 border-t border-gray-100">
                <span>SEMANA 1</span>
          ...
          <div className="bg-amber-50/50 p-3 rounded-2xl border border-amber-100/50 flex items-center gap-3.5 mt-4">
            <span className="p-1 rounded bg-[#C89B3C]/10 text-[#C89B3C] text-xs shrink-0 font-bold font-mono">INSIGHT</span>
            <p className="text-2xs text-[#0A2E5D] leading-relaxed font-sans m-0">
        ...
        <div className="lg:col-span-4 bg-white p-6 rounded-3xl border border-gray-150 text-left flex flex-col justify-between">
          <div className="space-y-4">
            <div className="border-b border-gray-100 pb-3">
              <span className="text-[9px] font-mono text-gray-400 uppercase font-black tracking-widest block">Eventos Críticos</span>
              <h3 className="text-sm font-serif font-bold text-[#0A2E5D] m-0">Fila Pendente de Ação</h3>
            </div>

            <div className="space-y-3.5">
              {alertsQueue.map((alertItem) => (
                <div key={alertItem.id} className="p-3 bg-gray-50/50 border border-gray-150 rounded-2xl space-y-1 hover:border-[#C89B3C]/20 transition-all text-left">
                  <div className="flex justify-between items-center text-[8px] font-mono text-[#C89B3C] font-bold">
                    ...
                    <span className="text-gray-400 font-semibold">{alertItem.time}</span>
                  </div>
                  <p className="text-2xs text-gray-600 leading-snug font-sans m-0">
                    {alertItem.text}
                  </p>
                </div>
              ))}
          ...
          <button
            ...
            className="w-full mt-4 py-2.5 bg-[#0A2E5D] text-white hover:bg-[#C89B3C] hover:text-[#0A2E5D] text-3xs font-mono font-bold uppercase rounded-xl tracking-widest border-0 cursor-pointer transition-all inline-flex items-center justify-center gap-2"
          >
