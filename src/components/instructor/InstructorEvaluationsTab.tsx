...
The Pre-Contracting Party shall indemnify, defend, and hold harmless the Host Entity from and against any and all claims, liabilities, losses, damages, costs, and expenses (including reasonable attorneys' fees under the regulations of Huambo bar) arising out of or resulting from any material breach of representational warranty, negligence, or willful omission in executing the oil extraction services.`,
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-3xl border border-gray-150">
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-3xs font-mono font-bold uppercase cursor-pointer border-0 ${
              activeSubTab === 'grade' ? 'bg-[#0A2E5D] text-white shadow' : 'text-gray-500 hover:bg-gray-50'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-3xs font-mono font-bold uppercase cursor-pointer border-0 ${
              activeSubTab === 'create' ? 'bg-[#0A2E5D] text-white shadow' : 'text-gray-500 hover:bg-gray-50'
            }`}
          >
        ...
        <span className="text-[10px] font-mono text-gray-400 font-bold uppercase hidden md:inline">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-8">
          ...
          {activeSubTab === 'grade' && (
            <div className="space-y-6">
              ...
              <div className="bg-white p-5 rounded-3xl border border-gray-150 space-y-4 text-left">
                <span className="text-[9px] font-mono text-gray-400 uppercase tracking-widest block font-bold border-b border-gray-100 pb-2">Seleccione o Rascunho Prático do Formando</span>
                    ...
                    <button
                      ...
                      className={`p-3 text-left rounded-2xl border transition-all cursor-pointer flex flex-col justify-between ${
                        selectedSubmissionId === sub.id
                          ? 'border-[#C89B3C] bg-[#0A2E5D]/5 text-[#0A2E5D]'
                          : 'border-gray-100 hover:border-gray-200 text-gray-700'
                      }`}
                    >
                      <div>
                        <h4 className="text-2xs font-serif font-black m-0 leading-tight">{sub.studentName}</h4>
                        <span className="text-[9px] font-mono text-gray-400 block mt-0.5">{sub.taskTitle}</span>
                      </div>
                  ...
                  return (
                    <div className="bg-white p-6 rounded-3xl border border-gray-150 space-y-6 text-left">
                      <div className="border-b border-gray-100 pb-4">
                        <span className="text-[9px] font-mono text-[#C89B3C] uppercase tracking-wider block font-bold">DRAFTING MANUSCRITO PARA DISCUSSÃO RECURSAL</span>
                        <h4 className="text-md font-serif font-black text-[#0A2E5D] mt-1 m-0">{currentSub.taskTitle}</h4>
                        <p className="text-2xs text-gray-400 font-mono mt-0.5">ESTUDANTE: {currentSub.studentName} ({currentSub.studentEmail})</p>
                      </div>

                      {/* Display paper paper skeuomorphic */}
                      <div className="p-5 sm:p-7 bg-[#FAF9F5] border-l-4 border-[#C89B3C] rounded-r-2xl font-mono text-xs text-gray-800 leading-relaxed shadow-inner select-text">
                      ...
                      <div className="bg-gray-50 p-5 rounded-2xl border border-gray-150 space-y-4">
                        <span className="text-[9.5px] font-mono text-gray-400 uppercase tracking-widest block font-bold">PARECER RECURSAL DO TUTOR</span>
                        
                        <div className="flex flex-col sm:flex-row gap-4 items-center">
                          <div className="w-full sm:w-1/3">
                            <label className="block text-[8px] font-mono text-gray-400 uppercase mb-1">Nota Quantitativa (0 - 100)</label>
                            <input
                              type="number"
                              min="0"
                              max="100"
                              ...
                              className="w-full p-2.5 text-xs bg-white rounded-xl border border-gray-200 font-serif font-black text-center text-slate-800"
                            />
                          </div>

                          <div className="w-full sm:w-2/3">
                            <label className="block text-[8px] font-mono text-gray-400 uppercase mb-1">Feedback Corretivo Individual</label>
                            <input
                              type="text"
                              ...
                              className="w-full p-2.5 text-xs bg-white rounded-xl border border-gray-200 text-slate-800"
                            />
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full py-2.5 bg-[#0A2E5D] hover:bg-[#C89B3C] hover:text-slate-900 border-0 text-white font-mono text-3xs font-black uppercase rounded-xl tracking-wider transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                        >
                          <CheckCheck size={14} />
              ...
              <div className="bg-white p-6 rounded-3xl border border-gray-150 space-y-4 text-left">
                <div>
                  <h4 className="font-serif font-black text-[#0A2E5D] text-sm m-0">Feedback e Aviso Coletivo (Todas as Turmas)</h4>
                  <p className="text-2xs text-gray-400 font-mono mt-0.5 uppercase">MURAL DE NOTAS DE MODERAÇÃO</p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-gray-100 hover:bg-[#C89B3C] text-gray-700 hover:text-slate-900 border-0 rounded-xl text-3xs font-mono font-bold uppercase tracking-wider transition-all cursor-pointer"
                  >
          ...
          {activeSubTab === 'create' && (
            <div className="bg-white p-6 rounded-3xl border border-gray-150 text-left space-y-6">
              <div>
                <h4 className="font-serif font-black text-[#0A2E5D] text-lg m-0">Criar Novo Instrumento de Avaliação</h4>
                <p className="text-xs text-gray-400 font-mono mt-0.5">GERADOR DE SESSÕES EXAMINADORAS</p>
              </div>

              <form onSubmit={handleRegisterAssessment} className="space-y-4">
                
                <div>
                  <label className="block text-[9px] font-mono font-bold uppercase text-gray-400 tracking-wider mb-1.5">Título Curricular da Prova</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full p-2.5 text-xs bg-gray-50 border border-gray-200 rounded-xl text-slate-800 focus:outline-none focus:border-[#C89B3C]"
                  />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[9px] font-mono font-bold uppercase text-gray-400 tracking-wider mb-1.5">Tipo de Instrumento</label>
                    <select
                      value={newType}
                  ...
                  <div>
                    <label className="block text-[9px] font-mono font-bold uppercase text-gray-400 tracking-wider mb-1.5 font-sans">Nota de Corte Mínima (0-100)</label>
                    <input
                      type="number"
                      min="50"
                      max="100"
                      ...
                      className="w-full p-2.5 text-xs bg-gray-50 border border-gray-200 rounded-xl font-serif font-bold text-center text-slate-800"
                    />
                  </div>

                  <div>
                    <label className="block text-[9px] font-mono font-bold uppercase text-gray-400 tracking-wider mb-1.5">Vinculo de Módulo</label>
                    <select
                      value={newScope}
                ...
                <button
                  type="submit"
                  className="w-full py-3 bg-[#0A2E5D] hover:bg-[#C89B3C] hover:text-slate-900 border-0 text-white font-mono text-3xs font-black uppercase rounded-xl tracking-wider transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                >
                  <PlusCircle size={14} />
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white p-5 rounded-3xl border border-gray-150 text-left space-y-4">
            <span className="text-[9px] font-mono text-gray-400 uppercase tracking-widest block font-bold border-b border-gray-100 pb-2">Banco de Provas Vigentes</span>
            
            <div className="space-y-3.5">
              {quizzesList.map((quiz, idx) => (
                <div key={idx} className="p-3 bg-gray-50/50 border border-gray-150 rounded-2xl text-left space-y-1">
                  <div className="flex justify-between text-[8px] font-mono text-[#C89B3C] font-bold">
                    <span>{quiz.type}</span>
                    <span className="text-gray-400 font-semibold">{quiz.weight}</span>
                  </div>
                  <h5 className="font-serif font-black text-[#0A2E5D] text-2xs m-0 leading-tight">
                    {quiz.title}
                  </h5>
                  <span className="block text-[8px] font-mono text-gray-400">Pontuação Mínima: {quiz.minGrade}/100</span>
                </div>
              ))}
            </div>
          </div>
